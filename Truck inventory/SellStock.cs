...
namespace Truck_inventory
{
    public partial class SellStock : Form
    {
        ...
        private void fillLocation()
        {
            ...
            {
                cbcustomer.Items.Add(dr["custName"]);
            }
        }
        ...
        private void btnSell_Click(object sender, EventArgs e)
        {
            ...
            if (cbcustomer.Text == "")
            {
                MessageBox.Show("Please select a customer", "Invalid Item", MessageBoxButtons.OK, MessageBoxIcon.Error);
                cbcustomer.Focus();
                return;
            }
            if (cbItemLocation.Text == "")
            {
                MessageBox.Show("Please select a truck.", "Invalid Item", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if ((Convert.ToInt32(txtQuantity.Text)) == (Convert.ToInt32(txtQuantityAvailable.Text)))
            {
                SqlCommand DeleteCommand = new SqlCommand("Delete from StockInHand where ItemCode = '" + cbItems.Text + "' and truckreg= '" + cbItemLocation.Text + "'", con);
            ...
            {
                SqlCommand UpdateCommand = new SqlCommand("Update StockInHand Set Quantity = Quantity - " + txtQuantity.Text + " where ItemCode = '" + cbItems.Text + "' and truckreg= '" + cbItemLocation.Text + "'", con);
                if (con.State == ConnectionState.Closed)
                {
                    con.Open();
                }
            ...
            SqlCommand myTransaction = new SqlCommand("Insert into StockTransactions values( '" + DateTime.Now.ToShortDateString() + "', '" + cbItems.Text.Trim() + "', '" + ds.Tables["StockInHand"].Rows[0]["ItemDescription"].ToString() + "', '" + cbItemLocation.Text.Trim() + "', '" + txtQuantity.Text + "', '" + cbcustomer.Text.Trim() + "')", con);
            if (con.State == ConnectionState.Closed)
            {
                con.Open();
            }
