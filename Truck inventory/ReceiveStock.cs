...
namespace Truck_inventory
{
    public partial class ReceiveStock : Form
    {
        ...
        {
            SqlDataAdapter adp = new SqlDataAdapter("Select truckreg from truck", con);
            DataSet ds = new DataSet();
            adp.Fill(ds, "truck");
            cbtruck.Items.Clear();
            foreach (DataRow dr in ds.Tables["truck"].Rows)
            {
                cbtruck.Items.Add(dr["truckreg"]);
            }
        }
        ...
        private void btnReceive_Click(object sender, EventArgs e)
        {
            ...
            if (rbSpecificItem.Checked == true)
            {
                ...
                if (cbtruck.Text == "")
                {
                    MessageBox.Show("Please select a truck", "Invalid truck", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    cbtruck.Focus();
                    return;
                }
                ...
                SqlDataAdapter adp_chkExistingItems = new SqlDataAdapter("Select * from StockInHand where ItemCode = '" + cbItem.Text + "' and truckreg = '" + cbtruck.Text + "'", con);
                ...
                {
                    SqlCommand UpdateCommand = new SqlCommand("Update StockInHand Set Quantity = Quantity + '" + txtQuantity.Text + "' WHERE ItemCode = '" + cbItem.Text + "' and truckreg = '" + cbtruck.Text + "'", con);
                ...
                {
                    SqlCommand InsertCommand = new SqlCommand("Insert into StockInHand values (@ItemCode, @Desc, @truck, @Rate, @Qty)", con);
                    ...
                    SqlParameter truck = new SqlParameter("@truck", SqlDbType.VarChar);
                    ...
                    InsertCommand.Parameters.Add(truck);
            ...
            {
                SqlDataAdapter adp2 = new SqlDataAdapter("Select ItemDescription, truck, ItemCode,  SUM(Quantity)as TotalQuantity from PODetails Where PONumber = '" + cbOrder.Text + "' group by ItemCode, ItemDescription, truck", con);
                ...
                {
                    SqlDataAdapter adp3 = new SqlDataAdapter("Select * from StockInHand Where ItemCode = '" + PODetails.Rows[i]["ItemCode"] + "' and truckreg = '" + PODetails.Rows[i]["truck"] + "'", con);
                    ...
                    if (rowCount1 > 0)
                    {
                        SqlCommand UpdateStock = new SqlCommand("Update StockInHand set Quantity = Quantity + " + PODetails.Rows[i]["TotalQuantity"] + " Where ItemCode = '" + PODetails.Rows[i]["ItemCode"] + "' and truckreg = '" + PODetails.Rows[i]["truck"] + "'", con);
                    ...
                    else
                    {
                        ...
                        newrow1[2] = PODetails.Rows[i]["truck"];
                        newrow1[3] = itemRate;
